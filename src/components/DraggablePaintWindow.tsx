...
type DraggablePaintWindowProps = {
  visible: boolean;
  onClose: () => void;
  onFocus?: () => void;
  zIndex?: number;
...
const INITIAL_VIEWPORT_WIDTH_RATIO = 0.55;
const INITIAL_VIEWPORT_HEIGHT_RATIO = 0.68;
...
export default function DraggablePaintWindow({
  visible,
  onClose,
  onFocus,
  zIndex,
}: DraggablePaintWindowProps) {
    ...
    const clampPosition = () => {
      ...
      setPosition((current) => ({
        x: Math.max(
          ...
          Math.min(
            current?.x ?? 0,
            window.innerWidth - (size?.width ?? element.offsetWidth),
          ),
        ),
        y: Math.max(
          0,
          Math.min(
            current?.y ?? 0,
            window.innerHeight - (size?.height ?? element.offsetHeight),
          ),
        ),
      }));
    };
  ...
  return (
    <div
      ...
      onPointerDown={(event) => {
        ...
        if (!target.closest("[data-paint-drag-handle]") || target.closest("button")) {
          return;
        }
      ...
      style={{
        ...
        width: size ? size.width : "55vw",
        height: size ? size.height : "68vh",
        zIndex,
      }}
    >
      <div
        ...
        <>
          {RESIZE_HANDLES.map((handle) => (
            <button
              key={handle.edge}
              type="button"
              aria-label={`Resize paint window from ${handle.edge}`}
              className={cn(
